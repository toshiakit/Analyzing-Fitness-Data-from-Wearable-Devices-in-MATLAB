...
% and wearable devices is about to become a mainstream as the internet
...
% "how well an activity was performed". Six subjects performed one set of 
...
% ensemble of decision trees (thus 'forest') trained on different subset 
% of data and uses majority votes of those trees to predict the result. 
% It tend to produce a highly accurate result, but the complexity of the 
...
% everything line up on a diagonal line, then you got 100% accuracy. 
...
%% Variable Importance
% One major criticism of Random Forest is that it is a black box algorithms
% and not easy to understand what it is doing. However, Random Forest can
% provide variable importance measure, which correspond to the change in
% prediction error with and without the presence of that variable in the
% model. 
%
...
% Receiver operating characteristic (ROC)> curves provides a convenient way
...
% Despite my initial misgivings about the data, we were able to high
% prediction accuracy with Random Forest model with just 12 features.
...
% for more details. I believe there will be an Android version of this
% webinar eventually. 
